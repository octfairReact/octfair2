...
import { IHirePost, IHirePostListResponse } from '../../../../../models/interface/IPost';
...
const HirePostMain = ({ searchParams }: { searchParams: { selectStatus: string; keyword: string } }) => {
  ...
  const getHirePostList = async () => {
    ...
    try {
      const response = await postApi<IHirePostListResponse>(HirePost.getList, param);
  ...
  const filtered = originalList?.filter((post) => {
    ...
    const keywordMatch = !keyword || post.title.toLowerCase().includes(keyword.toLowerCase());
          ...
          <Table hover>
            <thead>
              <tr>
                ...
                <th>
                  근무지역
                  <MyIcon height={19} onClick={() => sortList('workLocation')} />
                </th>
              </tr>
            </thead>
